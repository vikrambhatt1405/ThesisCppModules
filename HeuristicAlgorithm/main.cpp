...
int main(int argc, char **argv) {
    ...
    adjtocsr(&xadj, &adjncy, &adjwgt, &nvtxs, &nedges, "/mnt/sdb/data/walshaw/4elt.graph");
    part = new idx_t[nvtxs];
    METIS_PartGraphRecursive(&nvtxs, &ncon, xadj, adjncy, NULL, NULL, adjwgt, &nparts, NULL, NULL, NULL, &edgecut,
                             part);
...
//    std::cout<<pass<<" "<<edgecut<<std::endl;
    calculate_fiedler_values(part, xadj, adjncy, adjwgt, nvtxs, fiedler_value_x, fiedler_value_y);
    do {
...
//        std::cout<<pass<<" "<<edgecut<<std::endl;
        ...
        if (pq.size() < 5) {
            pq.push(cost_reduction);
        } else if (pq.size() >= 5) {
            pq.pop();
            pq.push(cost_reduction);
        }

    } while (cost_reduction > 0 || (pq.size() == 5 && pq.top() <= 30));
    return 0;
}
